...
def multi(load_path, param_path, epitope,model,epoch):
    save_p = param_path
    epitope = epitope
...
def multi_epitope(path):
    save_related = pd.read_csv(path, names=['load_path', 'param_path', 'epitope'])
    print(save_related)
    for i, load_path in enumerate(save_related['load_path']):

        epitope = save_related['epitope'][i]
        param_path = save_related['param_path'][i]
        multi(load_path, param_path, epitope, model1, n_epoch)

mcpas = 'McPAS_menu.csv'
vdjdb = 'VDJdb_munu.csv'
multi_epitope(mcpas)
multi_epitope(vdjdb)
